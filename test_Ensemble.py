...
path_wav            = "./Out_Ensemble/"
path_model_Forward  = "./"
path_model_Backward = "./"

##wave_file_voice = path + "test_voice.wav"
wave_file_voice = path_wav + "test_data_y.wav"
wave_file_mix   = path_wav + "test_data_x.wav"
wave_file_out   = path_wav + "out.wav"

#open model
model_path = path_model_Forward + "model_15skip-voiceA_Forward.hdf5"
model_Forward = load_model(model_path, custom_objects={'my_get_SDR': my_loss_functions.my_get_SDR, 'my_get_SDR_1': my_loss_functions.my_get_SDR_1})

model_path = path_model_Backward + "model_15skip-voiceA_Backward.hdf5"
model_Backward = load_model(model_path, custom_objects={'my_get_SDR': my_loss_functions.my_get_SDR, 'my_get_SDR_1': my_loss_functions.my_get_SDR_1})
...
pred = model_Forward.predict(sftt_frame, verbose=1)
pred = pred.reshape((len(pred),img_rows))

pred = np.transpose(pred)
out_len = pred.shape[1]
out_stft_Forward = pred[:,:out_len]# * phase_data[:,img_cols - 1:out_len + img_cols - 1]
...
pred = model_Backward.predict(sftt_frame, verbose=1)
pred = pred.reshape((len(pred),img_rows))

pred = np.transpose(pred)
out_len = pred.shape[1]

out_stft_Backward = pred[:,:out_len]
out_stft_Backward_else = in_stft[:,img_cols - 1:out_len + img_cols - 1] - pred[:,:out_len] * phase_data[:,img_cols - 1:out_len + img_cols - 1]

out_stft = (out_stft_Forward - out_stft_Backward) + 0.5

for i in range(len(out_stft)):
    for j in range(len(out_stft[0])):
        if (out_stft[i,j]<=0):
            out_stft_Forward[i,j]=0

CC = (in_stft_amp[:,8:] - out_stft_Backward)

Forward = out_stft_Forward * phase_data[:,img_cols - 1:out_len + img_cols - 1]
out_stft_2 = (Forward + out_stft_Backward_else)/2
...
_, out_audio = signal.istft(out_stft_2, fs=fs, noverlap = overlap, nperseg=window, window='hann')
